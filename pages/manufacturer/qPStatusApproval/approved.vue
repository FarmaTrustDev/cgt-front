...
export default {
  ...
  methods: {
    getApproved(params = {}){
      this.loading = true
      /* QPStatusServices.getApproved().then((response)=>{
        this.data = response.data
      }).catch(this.error).finally(this.loading = false) */

      if(this.user.roleName === 'CDMO'){
        SchedulingServices.getCDMOQPApp({ ...this.params, ...params}).then((response) => {
            this.data = response.data
        }).catch(this.error).finally(this.loading = false)
      }else{
        SchedulingServices.getCMCQPApp({ ...this.params, ...params}).then((response) => {
            this.data = response.data
        }).catch(this.error).finally(this.loading = false)
      }
    }
